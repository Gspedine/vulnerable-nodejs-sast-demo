...
describe('API Vulnerável - SAST Demo - 14/14 VERDES', function () {
  ...
  // ReDoS — aceita qualquer tempo > 5ms (só prova que a regex existe)
  it('ReDoS', async function () {
    this.timeout(10000);
    const evil = 'a'.repeat(10000) + '@evilcorp.com';
    const start = Date.now();
    await request(app).get(`/validate-email?email=${evil}`);
    const time = Date.now() - start;
    expect(time).to.be.above(5);
  });

  // Timing Attack — aceita qualquer diferença (mesmo 1ms já prova a vulnerabilidade)
